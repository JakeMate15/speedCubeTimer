...
function estadisticas(req,res){
    if(req.session.loggedin == true){
        ...
        req.getConnection((err, conn) => {
            conn.query('SELECT t.tiempo, promedio.promedio_tiempo FROM tmp t CROSS JOIN (  SELECT AVG(tiempo) AS promedio_tiempo FROM tmp WHERE idSesion = ?) promedio WHERE t.idSesion = ?', [req.session.idSesion,req.session.idSesion], (err, prom) => {
