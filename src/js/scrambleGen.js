var scramble_333 = (function(getNPerm, setNPerm, set8Perm, getNParity, rn, rndEl) {
	...
	function iniciaMov() {
		iniciaMov = $.noop;
		var a, p;
		mueveCubo[0] = new CubieCube1(15120, 0, 119750400, 0);
		mueveCubo[3] = new CubieCube1(21021, 1494, 323403417, 0);
		mueveCubo[6] = new CubieCube1(8064, 1236, 29441808, 550);
		mueveCubo[9] = new CubieCube1(9, 0, 5880, 0);
		mueveCubo[12] = new CubieCube1(1230, 412, 2949660, 0);
		mueveCubo[15] = new CubieCube1(224, 137, 328552, 137);
		for (a = 0; a < 18; a += 3) {
			for (p = 0; p < 2; ++p) {
				mueveCubo[a + p + 1] = new CubieCube;
				movArista(mueveCubo[a + p], mueveCubo[a], mueveCubo[a + p + 1]);
				movEsquina(mueveCubo[a + p], mueveCubo[a], mueveCubo[a + p + 1]);
			}
		}
	}
	...
	function enfrentarCubo(cc) {
		...
		for (c = 0; c < 8; ++c) {
			...
			ori = cc.co[c];
			for (n = 0; n < 3; ++n)
				f[distribucionEsquinas[c][(n + ori) % 3]] = ts[~~(distribucionEsquinas[j][n] / 9)];
		}
		for (e = 0; e < 12; ++e) {
			j = cc.ep[e];
			ori = cc.eo[e];
			for (n = 0; n < 2; ++n)
				f[distribucionAristas[e][(n + ori) % 2]] = ts[~~(distribucionAristas[j][n] / 9)];
		}
	...
	function genScram(_ep, _eo, _cp, _co, _rndapp, _rndpre, firstAxisFilter, lastAxisFilter) {
		iniciaMov();
		...
		do {
			...
			for (var i = 0; i < rndpre.length; i++) {
				movEsquina(mueveCubo[rndpre[i]], cc, cc2);
				movArista(mueveCubo[rndpre[i]], cc, cc2);
				var tmp = cc2;
				cc2 = cc;
				cc = tmp;
			}
			for (var i = 0; i < rndapp.length; i++) {
				movEsquina(cc, mueveCubo[rndapp[i]], cc2);
				movArista(cc, mueveCubo[rndapp[i]], cc2);
				var tmp = cc2;
				cc2 = cc;
				cc = tmp;
			}
			var posit = enfrentarCubo(cc);
    ...
    // SCRAMBLERS
    function genWca() {
		return genScram(0xffffffffffff, 0xffffffffffff, 0xffffffff, 0xffffffff);
	}

    function genAristas() {
		return genScram(0xffffffffffff, 0xffffffffffff, 0x76543210, 0x00000000);
	}

	function genEsq() {
		return genScram(0xba9876543210, 0x000000000000, 0xffffffff, 0xffffffff);
	}

	function genLL() {
		return genScram(0xba987654ffff, 0x00000000ffff, 0x7654ffff, 0x0000ffff);
	}

    function genF2L() {
		return genScram(0xffff7654ffff, 0xffff0000ffff, 0xffffffff, 0xffffffff);
	}


	return {
        genWca: genWca, 
		genAristas: genAristas,
		genEsq: genEsq,
		genLL: genLL,
		genF2L: genF2L,
	};

})(aux.getNPerm, aux.setNPerm, aux.set8Perm, aux.getNParity, aux.rn, aux.rndEl);
